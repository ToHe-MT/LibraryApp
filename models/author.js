...
AuthorSchema.virtual("birth_date").get(function () {
    ...
    if (this.date_of_birth) {
        ...
        result = `${this.date_of_birth.getFullYear()}-${currentMonth}-${currentDate}`;
    }
    return result;
});
...
AuthorSchema.virtual("death_date").get(function () {
    ...
    if (this.date_of_death) {
        ...
        result = `${this.date_of_death.getFullYear()}-${currentMonth}-${currentDate}`;
    }
    return result;
});
...
AuthorSchema.virtual("lifespan").get(function () {
    ...
    return ''; // Return an empty string when there's no birth or death date
});
